<%@ Page Language="VB" AutoEventWireup="false" MasterPageFile="~/MasterPage.master" CodeFile="edo_cta_fte_cv.aspx.vb" Inherits="Admin_procesos_edo_cta_fte_cv" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">    
    ...
    function fecha_hoy() {
        ...
        var day = d.getDate();
        return (day < 10 ? '0' : '') + day + '_' + (month < 10 ? '0' : '') + month + '_' + d.getFullYear();
    }
    ...
    function MostrarEdoCtaFte(filtro) {
       ...
       .navButtonAdd('#pager',
           {
               caption: "Alta de Dcto. Det.",
               buttonicon: "ui-icon-document",
               onClickButton: function () {
                   var myGrid = $('#grid');
                   selectedRowId = myGrid.jqGrid('getGridParam', 'selrow');
                   cellfid = myGrid.jqGrid('getCell', selectedRowId, 'fid');
                   cellfecvenc = myGrid.jqGrid('getCell', selectedRowId, 'FEC_VENC');
                   cellfecpav = myGrid.jqGrid('getCell', selectedRowId, 'FEC_PAV');
                   var frase_usuario = $("#lblUsuario").html();
                   var usu = frase_usuario.split(":");
                   if (cellfid) {
                       MostrarModalDet(cellfid,usu[2],0,cellfecvenc,cellfecpav);                      
                   }
                   else {
            ...
            $.ajax(
             {
                 url: "../../WebServices/WebServiceEdoCta.asmx/GetEdoCtaFte_cv", //PageMethod
    ...
    $(document).ready(function () {
      ...
      /*  $(".posicion_boton_cargar_pagos").click(function (e) { //Actualizar datos en el registro            
            var url1 = "../../Procesos/cargar_pagos.html";
            newWindow = window.open(url1, "_blank", "top=30,left=200,menubar=no,toolbar=no,location=no, resizable=no,height=800,width=1000,status=no,scrollbars=yes,minimizable=no,maxmizable=no,resizable=0,titlebar=no,modal=yes");
            if (newWindow.focus()) {
                newWindow.focus()
            }
        });*/

      $(".posicion_boton_accion_det").click(function (e) { //Actualizar datos en el registro            
          e.preventlogin();

          if (ValidarDeterminante()) {
              alert('Guardar determinante');
          }
      });


      $(".window .posicion_boton_cancelar_det").click(function (e) {
    ...
    function ValidarDeterminante() {
        var valid = true;
        var accion = $("#lblaccionpae1").text();



        if (accion == 0) { /*Si el usuario escogió dar de alta un nuevo levantamiento*/            
            valid = valid && isDate($("#txt_fec_imp_dcto_det").val());
            if (valid) {                
                valid = valid && isDate($("#txt_fec_ent_dcto_det").val());
                if (valid) {                    
                    valid = valid && isDate($("#txt_fec_notif_det").val());
                    if (valid) {
                        var fec_imp_dcto = ConvertirCadenaToFecha($("#txt_fec_imp_dcto_det").val());
                        var fec_ent_notific = ConvertirCadenaToFecha($("#txt_fec_ent_dcto_det").val());
                        var fec_notif = ConvertirCadenaToFecha($("#txt_fec_notif_det").val());
                        var fecpav = ConvertirCadenaToFecha($("#lblfecpav").text());
                        var fecvenc = ConvertirCadenaToFecha($("#lblfecvenc").text());

                        if (fec_imp_dcto > fecpav && fec_imp_dcto > fecvenc) {   /*Validación de fechas*/
                            if (fec_ent_notific > fec_imp_dcto) {
                                if (fec_notif_det > fec_ent_notific) {
                                    if ($("#filedet").val().length > 0) {              /*Valida si existe archivo capturado en el control input file*/
                                        if (ValidarArchivo($("#filedet")[0].files[0])) { /*Valida que el archivo sea de tipo PDF y tenga un tamaño menor a 1 MB*/
                                            valid = valid && true;
                                        }
                                        else {
                                            $("#filedet").val("");
                                        ...
                                        if ($("#archivo").text() != "" && $("#archivo").text() != "Ningún archivo seleccionado") {     /*Si la propiedad TEXT de la etiqueta que contiene el nombre de archivo contiene algo significa que la interfaz esta en estatus de edición, por lo tanto se regresará TRUE porque significa que no se desea cambiar el archivo*/
                                            valid = valid && true;
                                        }
                                        else {
                                    ...
                                    valid = valid && false;
                                    alert('Fecha de notificación errónea, no puede ser menor a la fecha de que se entregó al ministro');
                                }
                            }
                            else {
                                valid = valid && false;
                                alert("Fecha de entrega al ministro errónea, no puede ser mayor a la fecha de impresión del documento");
                            }
                        }
                        else {
                            valid = valid && false;
                            alert("Fecha de impresión del documento errónea debe ser mayor a la fecha de entrega de obra y también mayor la fecha de vencimiento de pagos. ");
                        }
                    }
                    else {
                        valid = valid && false;
                        alert('Fecha de notificación del documento determinante errónea');
                    }
                }
                else {
                    valid = valid && false;
                    alert('Fecha de entrega del documento al ministro errónea');
                }
            }
            else {
                valid = valid && false;
                alert('Fecha de impresión del documento errónea');
            }
        }
       ...
       if (accion == 1 && estanu == 2) {                    
        ...
        if (accion == -1) {
            alert("Has seleccionado eliminar la determinación.");
            valid = true;
        }
    ...
    function MostrarModalDet(fid,idusu,accion,fecvenc,fecpav) {

        $("#fid").text(fid);
        $("#lblidusu").text(idusu);
        $("#lblaccionpae1").text(accion);
        $("#lblfecvenc").text(fecvenc);
        $("#lblfecpav").text(fecpav);        
        ...
        $("#dialogodet").css('top', winH / 2 - $("#dialogodet").height() / 2);
        $("#dialogodet").css('left', winW / 2 - $("#dialogodet").width() / 2);

        //transition effect
        $("#dialogodet").fadeIn(2000);
    }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">  
    ...
    <b>Cooperador:<input type="text" name="Cooperador:" id="txtcoop" style="width:80px"/>&nbsp &nbsp Nombre de Cooperador:<input type="text" name="NomCoop" id="txtnomcoop" style="width:250px"/>&nbsp &nbsp Frente:</><input type="text" name="fid" id="txtfid" style="width:50px" /> </b><br/><br/>
<table id="grid"></table>
<div id="pager"></div>&nbsp 
 
<div id="boxes">        
    <div id="dialogodet" class="window"> 
       <div id="titulodet"> 
         <p style="text-align:center">INICIO DEL PROCEDIMIENTO DE ADMINISTRACIÓN DE EJECUCIÓN (PAE)</p>
         No. Frente: <label for="fid" id="fid"></label><br/>                           
         No. Usuario: <label for="idusu" id="lblidusu"></label><br/>
         Acción:<label for="accion" id="lblaccionpae1"></label><br/><br/> 
         Fecha de Vencimiento de Pagos: <label for="fecvenc" id="lblfecvenc"></label><br/>
         Fecha de entrega de obra: <label for="fecpav" id="lblfecpav"></label><br/><br/>          
       </div>
       <div class="datos" id="bodypresup">        
          <label for="name">*Fecha de impresión del Dcto. Determinante:</label><input type="text" name="fec_imp_dcto_det" id="txt_fec_imp_dcto_det" size="10"/><br/><br/>                              
          <label for="name">*Fecha de entrega del Dcto. al notificador:</label><input type="text" name="fec_ent_dcto_det" id="txt_fec_ent_dcto_det" size="10"/><br/><br/>
          <label for="name">*Fecha de notificación del Dcto. Determinante:</label><input type="text" name="fec_notif_det" id="txt_fec_notif_det" size="10"/><br/><br/>          
          <label for="name">*Documento Determinante:</label><input type="file" id="filedet" name="file"/> <a href="#" id="archivo" class="verarchivo"/>                    
       </div>                  
       <div class="botonesA" id="botonesC">
          <p style="text-align:right"><a href="#" class ="posicion_boton_accion_det">Aceptar</a>&nbsp&nbsp&nbsp<a href="#" class="posicion_boton_cancelar_det">Cancelar</a></p>                
       </div>
    </div>     
    <div id="mask"></div>
 </div>
