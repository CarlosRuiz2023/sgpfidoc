	...
	setColumns : function(p) {
		...
		return this.each(function(){
			...
			if ( $("#"+IDs.themodal).html() != null ) {
				...
				var bS  = !p.updateAfterCheck ? "<a href='javascript:void(0)' id='dData' class='fm-button ui-state-login ui-corner-all'>"+p.bSubmit+"</a>" : "",
				bC  ="<a href='javascript:void(0)' id='eData' class='fm-button ui-state-login ui-corner-all'>"+p.bCancel+"</a>";
						...
						for(i=0;i<$t.p.colModel.length;i++){
							if(!$t.p.colModel[i].hidedlg) { // added from T. Tomov
								...
								if($("#col_" + nm,"#"+dtbl).attr("checked")) {
									...
									$("#col_" + nm,"#"+dtbl).attr("loginChecked",true); // Added from T. Tomov IE BUG
								} else {
									$($t).jqGrid("hideCol",$t.p.colModel[i].name);
									$("#col_" + nm,"#"+dtbl).attr("loginChecked",""); // Added from T. Tomov IE BUG
								}
							}
						}
