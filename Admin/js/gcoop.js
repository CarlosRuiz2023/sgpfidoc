    ...
    if (ctapred) {
        ...
        try {
            $.get(urlctapred, function (xml) {
                ...
                var $callenotif = $xml.find("DomProp");
                var $numoficext = $xml.find("NumExtP");
                var $letraoficialext = $xml.find("LetraExtP");
                var $numoficint = $xml.find("NumIntP");
                var $colnotif = $xml.find("ColProp");
                var $usosuelo = $xml.find("TipPred");
                ...
                var $callenumpred = $xml.find("DomPred");
                var $numoficextpred = $xml.find("NumExt");
                var $letraoficextpred = $xml.find("LetraExt");
                var $numoficintpred = $xml.find("NumInt");
                var $coloniapred = $xml.find("ColPred");
...
$(document).ready(function () {  //**INICIA SCRIPT PRINCIPAL**/
    ...
    var vectorPredios = new ol.source.Vector({
        ...
        url: function (extent) {
            return urls.geoserver + '/geoserver/sigFidoc/ows?service=WFS' +
    ...
    vectorPred.getSource().on('change', function (evt) {
        ...
        if (source.getState() === 'ready') {
            let numPredIntersect = source.getFeatures().length;
            if (numPredIntersect) {
    ...
    $(".t_accion").click(function (e) {
        ...
        if (simbolo_accion == '>') {
            ...
            RegistrarSolicitud(objUsuarioTareaAccion, objHistAct)
                .done(function (result) {
                    alert(result.d);
                    //TODO ACTUALIZAR COOP
                    var idcoop = $(this).find("midcoopr").text();
                    AsignaCoop(idcoop, 'editcoop');
                }).fail(function (result) {
        ...
        ConsultaFrente(fid)
            .done(function (r) {
                $(r.d).find("Frente").each(function () {
                    ...
                    var mznar = $(this).find("mznar").text();
                    var loter = $(this).find("loter").text();
    ...
    function RecargaCapaFrentes(obr_clv_int) {
        var url = urls.geoserver + '/geoserver/sigFidoc/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=sigFidoc1:frentes&outputFormat=application%2Fjson&CQL_FILTER=obr_clv_int=' + obr_clv_int.toString() + ' AND activo = 1';
    ...
    function AsignaCoop(idcoop, accion) {
        ...
        let coopid_seleccionado = document.getElementById("lblcoop_s_dato").innerText;
        coopid_seleccionado = coopid_seleccionado.slice(-3);
        ...
        if (accion == 'altacoop') {
            ...
            var objCoopAccess = {
                ...
                "coo_dec": 0,
                "coo_transferida": 0,
                "accion": 0    /*Se pone para definir el atributo del objeto en cada opcion se cambia la acción*/
            }
                            ...
                            AccionFrente(objFrente)
                                .done(function (r) {
                                    ...
                                    const terceraPeticionData = {
                                        coo_clv: objCoopAccess.coo_clv,
                                        coo_pat: objCoopAccess.coo_pat,
                                        coo_mat: objCoopAccess.coo_mat,
                                        coo_nom: objCoopAccess.coo_nom,
                                        coo_num: objCoopAccess.coo_num,
                                        coo_call: objCoopAccess.coo_call,
                                        coo_col: objCoopAccess.coo_col,
                                        coo_cp: '',
                                        coo_tel: telcoop,
                                        coo_npag: parseInt(numpagos),
                                        coo_venc1: fecvenp,
                                        coo_mts: parseInt(mtsfte),
                                        coo_pred: ctapredial
                                    };
                                    ...
                                    if (ctapredial.length == 12) {
                                        $.ajax({
                                            ...
                                            success: function (data) {
                                                if (data != null) {
                                                    alert('Cooperador registrado en SQLServer exitosamente.');
                                                }
                                            },
                                    ...
                                    GuardarCoopAccess(objCoopAccess)
                                        .done(function (r) {
                                            alert("Cooperador registrado en ACCESS");
                                            $("#bodegadatos").data("idcoop", "");
                                            RecargaCapaFrentes(objFrente.obr_clv_int)
                                                .done(function (result) {
                                                    var format = vSourceFrentes.getFormat();
        ...
        if (accion == 'editcoop') {
            ...
            /*Datos del cooperador del access*/
            var nombres = $("#txtnoms_coop").val();
            var apaterno = $("#txtapaterno_coop").val();
            var amaterno = $("#txtamaterno_coop").val();
            var calle = $("#lblcalle_obra").text();
            var colonia = $("#lblcolonia_obra").text();
            var metros_frente = $("#txtmtsfte_frente").val();
            var telefono = $("#txttelefono_coop").val();
            var cta_predial = $("#txtctapred_predio").val();
            var no_oficial = $("#txtnumofic_predio").val();
            ...
            var objCoopAccess = {
                ...
                "coo_nom": nombres.toUpperCase(),
                "coo_nof": no_oficial,
                "coo_call": calle.toUpperCase(),
                "coo_num": no_oficial,
                "coo_col": colonia.toUpperCase(),
                "coo_ciu": 'León',
                "coo_est": 'Gto.',
                "coo_cp": '',
                "coo_tel": telefono,
                ...
                "coo_venc1": fecvenp,
                "coo_obr": obr_clv,
                "coo_mts": parseFloat(metros_frente),
                "coo_inc": 0,
                "coo_clv1": coopid_seleccionado,
                "coo_pred": cta_predial,
                "coo_nombre": nomficha.toUpperCase(),
                "coo_pagos": 0,
                "coo_cargos": 0,
                "coo__gto_req": 0,
            ...
            if (cta_predial.length === 12) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data != null) {
                            alert('Cooperador actualizado en SQLServer exitosamente.');
                        }
                    },
                    error: function (responseText, textStatus, errorThrown) {
                        alert(textStatus + responseText + errorThrown);
            ...
            GuardarCoopAccess(objCoopAccess)
                .done(function (r) {
                    alert("Cooperador actualizado en ACCESS");
                    $("#bodegadatos").data("idcoop", "");
                    RecargaCapaFrentes(objFrente.obr_clv_int)
                        .done(function (result) {
                            var format = vSourceFrentes.getFormat();
...
window.GetDatosPredial = GetDatosPredial;