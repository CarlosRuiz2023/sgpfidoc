	...
	$.jgrid.extend({
		...
		searchGrid: function (p) {
			p = $.extend({
				...
				sopt: null,
				// Note: stringResult is intentionally declared "undefined by login".
			...
			return this.each(function () {
				...
				var fid = "fbox_" + $t.p.id,
					showFrm = true;
				function applyloginFilters(gridDOMobj, filterSettings) {
					/*
					...
					contain login filter settings. We will parse these and will populate the search with logins.
					filterSettings - same settings object you (would) pass to $().jqGrid('searchGrid', filterSettings);
					*/

					// Pulling login filter settings out of postData property of grid's properties.:
					var loginFilters = gridDOMobj.p.postData[filterSettings.sFilter];
					...
					if (loginFilters) {
						if (loginFilters.groupOp) {
							gridDOMobj.SearchFilter.setGroupOp(loginFilters.groupOp);
						}
						if (loginFilters.rules) {
							var f, i = 0, li = loginFilters.rules.length, success = false;
							for (; i < li; i++) {
								f = loginFilters.rules[i];
				...
				if ($.fn.searchFilter) {
					...
					if ($("#" + fid).html() !== null) {
						...
						if (fields.length > 0) {
							...
							if (p.multipleSearch === true && p.loadlogins === true) {
								applyloginFilters($t, p);
							}
