...
Public Class WebServiceTipoObra
    Inherits System.Web.Services.WebService


    <WebMethod(Description:="Actualizar el registro del documento")> _
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json, XmlSerializeString:=True)>
    Public Function GetTipoObra(accion As Integer, tipoobra As Integer) As String
        ...
        Using con As New NpgsqlConnection(constr)
            Using cmd As New NpgsqlCommand("public2.sp_tipoobraq", con)
                Dim adapt As NpgsqlDataAdapter = New NpgsqlDataAdapter(cmd)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("@accionx", accion)
                cmd.Parameters.AddWithValue("@tipoobrax", tipoobra)
                adapt.Fill(ds, "tipoobra")
                Try
